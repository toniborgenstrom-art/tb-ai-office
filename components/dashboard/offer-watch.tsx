...
export function OfferWatch({ offers }: { offers: WatchedOffer[] }) {
  ...
  async function syncHilma() {
    ...
    else { setNotice(`Hilma tarkistettu: ${data.found} ilmoitusta, ${data.relevant ?? 0} sopivaa, ${data.imported} uutta ja ${data.updated ?? 0} täydennetty.`); router.refresh(); }
    setSyncing(false);
  }
  ...
  return <>
    ...
    {notice && <p className="mt-5 border border-[#b8d9c9] bg-[#edf8f2] p-3 text-sm text-[#1f6b56]">{notice}</p>}
