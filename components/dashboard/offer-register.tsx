...
export type RegisteredOffer = {
  ...
  status: string; source: string; projectName: string; description: string; region: string; sourceUrl: string;
};
...
export function OfferRegister({ offers }: { offers: RegisteredOffer[] }) {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/offers/${editing.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
      ...
      source: form.get("source"), region: form.get("region"), sourceUrl: form.get("sourceUrl"), description: form.get("description")
    }) });
