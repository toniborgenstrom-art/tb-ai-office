import { syncHilmaForCompany } from "@/lib/hilma/sync";
import { getCurrentWorkspace } from "@/lib/supabase/current-user";
import { NextResponse } from "next/server";

export async function POST() {
  const { companyId, error } = await getCurrentWorkspace();
  if (error || !companyId) return NextResponse.json({ error }, { status: 401 });
  try {
    return NextResponse.json({ ok: true, ...(await syncHilmaForCompany(companyId)) });
  } catch (cause) {
